...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // Full-Text Search on Content (برای جستجوی داخل کتاب)
        DB::statement('CREATE INDEX book_contents_text_fulltext_idx ON book_contents USING gin(to_tsvector(\'english\', COALESCE(text, \'\')))');

        // Partitioning by book_id for better performance (Optional but recommended for large scale)
        // این را در آینده می‌توانید فعال کنید:
        /*
        DB::statement('
    ...
    {
        DB::statement('DROP INDEX IF EXISTS book_contents_text_fulltext_idx');
